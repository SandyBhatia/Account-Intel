...
```powershell
$src = "$env:USERPROFILE\Downloads\account-intel-brief-1.3"   # where you unzipped
...
see `brief-1.3`. That is the build stamp — if it is missing, the new code is
...
Wait for Vercel to finish, then load the deployed site and confirm `brief-1.3`
